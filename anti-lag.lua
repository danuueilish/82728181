if not _G.__ANTI_LAG_FISHING_LOADED then
    ...
    local function startZoomLoop()
        if _G.AntiLagZoomLoopConn then return end
    ...
    local function createOverlay()
        if _G.AntiLagOverlayGui and _G.AntiLagOverlayGui.Parent then return end
    ...
    local function runStaticCleanup()
        if _G.__ANTI_LAG_STATIC_DONE then return end
        ...
        for _, part in ipairs(ws:GetDescendants()) do
            if part:IsA("BasePart") then
                if not part:IsDescendantOf(LP.Character or Instance.new("Folder")) then
    ...
    local function hookRod()
        if _G.AntiLagRodConn then return end
        local Rod = findRodRemote()
        if not Rod then
            task.spawn(function()
                while _G.AntiLagEnabled and not Rod do
                    Rod = findRodRemote()
                    task.wait(1)
                end
                if Rod and not _G.AntiLagRodConn and _G.AntiLagEnabled then
                    _G.AntiLagRodConn = Rod.OnClientEvent:Connect(function(arg1, arg2)
                        if not _G.AntiLagEnabled then return end
                        if arg1 == "Zoom" then
    ...
    function _G.EnableAntiLag()
        if _G.AntiLagEnabled then return end
    ...
    function _G.DisableAntiLag()
        ...
        -- balikin attr fishing animation
