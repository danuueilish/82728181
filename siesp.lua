...
local function getAnimalStatus(label)
    if not label then return "[?]", Color3.fromRGB(150, 150, 150) end
    local raw  = tostring(label.Text or "")
    local upper = raw:upper()

    if upper:find("EATING") or upper:find("MAKAN") then
        local secs = raw:match("(%d+)s") or raw:match("(%d+)")
        if secs then
            return string.format("[Eating %ss]", secs), Color3.fromRGB(220, 170, 50)
        end
        return "[Eating]", Color3.fromRGB(220, 170, 50)

    elseif upper:find("HUNGRY") or upper:find("LAPAR") then
        return "[Hungry]", Color3.fromRGB(255, 80, 80)

    elseif upper:find("READY") or upper:find("SIAP") then
        return "[Ready!]", Color3.fromRGB(80, 255, 120)

    else
        local secs = raw:match("(%d+)s")
        if secs then return string.format("[%ss]", secs), Color3.fromRGB(220, 170, 50) end
        if raw ~= "" then return string.format("[%s]", raw), Color3.fromRGB(200, 200, 200) end
        return "[?]", Color3.fromRGB(150, 150, 150)
    end
end

local function clearAnimalESPs()
    for _, esp in pairs(animalESPObjects) do
        pcall(function() if esp and not esp.Deleted then esp:Destroy() end end)
    end
...
local function setupAnimalESPs()
    ...
    if coop then
        for i = 1, 12 do
            ...
            if esp then table.insert(animalESPObjects, esp) end
        end
    end
    if barn then
        for i = 1, 15 do
...
local function startAnimalESP()
    if animalESPActive then return end
    animalESPActive = true
    setupAnimalESPs()
    task.spawn(function()
